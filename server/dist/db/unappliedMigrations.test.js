...
describe('findUnappliedMigrations', () => {
    const done = (name) => ({ migration_name: name, finished_at: '2026-09-24T00:00:00Z', rolled_back_at: null });
    it('reports migrations in this tree that the database never applied', () => {
        assert.deepEqual(findUnappliedMigrations(['001_init', '002_calibration', '003_payload'], [done('001_init')]), ['002_calibration', '003_payload']);
    });
    ...
    it('does not count a migration that started but never finished', () => {
        assert.deepEqual(findUnappliedMigrations(['001_init'], [{ migration_name: '001_init', finished_at: null, rolled_back_at: null }]), ['001_init']);
    });
    it('does not count a migration that was rolled back', () => {
        assert.deepEqual(findUnappliedMigrations(['001_init'], [{ ...done('001_init'), rolled_back_at: '2026-09-24T00:01:00Z' }]), ['001_init']);
    });
    it('reads the epoch-millisecond integers SQLite actually stores', () => {
        assert.deepEqual(findUnappliedMigrations(['001_init', '002_calibration'], [
    ...
    it('counts a migration applied on a later attempt after an earlier one failed', () => {
        assert.deepEqual(findUnappliedMigrations(['001_init'], [
...
//# sourceMappingURL=unappliedMigrations.test.js.map